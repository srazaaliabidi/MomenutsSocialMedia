...
function addPostImage(userID, title, contentURL, caption, endFunction) {
	var date = new Date();
	var query = "INSERT INTO Post (userID, title, type, contentURL, caption, dateCreated) VALUES ('"+userID+"', '"+title+"', 'text', '"+contentURL+"', '"+caption+"', '"+date+"');";
	connection.query(query, function (error, result) {
		if (error) {
			console.log(error);
			endFunction("0");
...
router.post('/newUser', function (req, res) {
	console.log("/newUser");
	newUser(req.body.email, req.body.username, req.body.password, req.body.firstName, req.body.lastName, req.body.city, req.body.state, req.body.DOB, req.body.pfpURL, function(user, id) {
...
router.post('/newPostImage', function (req, res) {
	...
	if (!req.session.uid) {
		...
		addPostImage(req.session.uid, req.body.title, req.body.contentURL, req.body.caption, function(output) {
			res.send(output);
		});  
	}
});
